...
// NE: I would like to use some of the stuff in the norm branch, but first merge. 
 ...
 scanner_geometry.replicated_modules.push_back(rep_module);
  ...
  // test
  {
    ...
    if (!stir::is_null_ptr(dynamic_cast<const stir::ProjDataInfoBlocksOnCylindricalNoArcCorr *>(&stir_proj_data_info))){
      auto& pdi = dynamic_cast<const stir::ProjDataInfoBlocksOnCylindricalNoArcCorr &>(stir_proj_data_info); // cast to blocks
      for (unsigned int r=0; r< static_cast<unsigned>(stir_scanner->get_num_rings()); ++r)
      ...
      }else if (!stir::is_null_ptr(dynamic_cast<const stir::ProjDataInfoCylindricalNoArcCorr *>(&stir_proj_data_info))){
        auto& pdi = dynamic_cast<const stir::ProjDataInfoCylindricalNoArcCorr &>(stir_proj_data_info); // cast to cylindrical
        for (unsigned int r=0; r< static_cast<unsigned>(stir_scanner->get_num_rings()); ++r)
          for (unsigned int d=0; d < static_cast<unsigned>(stir_scanner->get_num_detectors_per_ring()); ++d)
            {
        ...
        stir::error("Cannot only handle projdatainfo");
      }
  }
